...
exports.Insert = function(item1, item2, item3, item4, item5, item6, item7, item8, item9, item10, item11, item12, item13, item14, item15, item16, item17, item18, item19, item20, item21, item22, item23) {
    mediumdutydieselvehicles .find({ fleetName: item2, email: item1 }).exec(function(err, results) {
        var length = results.length;
        console.log(length);
        if (length == 0) {
            var newpassen = new mediumdutydieselvehicles ({
                email: item1,
                fleetName: item2,
...
exports.delete = function(item1, item2) {
    mediumdutydieselvehicles .findOneAndDelete({ fleetName: item2, email: item1 }, function(err, results) {

    });
}
exports.change = function(item1, item2) {
    mediumdutydieselvehicles .findOne({ fleetName: item2, email: item1 }, function(err, doc) {
        if (err) throw err;
        else
            return doc;
    });
