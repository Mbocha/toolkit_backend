...
exports.Insert = function(item1, item2, item3, item4, item5, item6, item7, item8, item9, item10, item11) {
    passengerelectricvehicles.find({ fleetName: item1, email: item2 }).exec(function(err, results) {
        var length = results.length;
        console.log(length);
        if (length == 0) {
            var newpass = new passengerelectricvehicles  ({
                email: item2,
                fleetName: item1,
...
exports.delete = function(item1, item2) {
    passengerelectricvehicles.findOneAndDelete({ fleetName: item2, email: item1 }, function(err) {
